...
// Helper to normalize product id comparison across ObjectId/string fixtures
const isSameProduct = (item, productId) => {
  const id = item && item.product && (item.product._id || item.product.id || item.product);
...
// Update cart items respecting the "quantity === 0 removes item" rule
const updateCartItems = (cartItems, productId, quantity, productDoc) => {
  if (quantity === 0) {
    return cartItems.filter((item) => !isSameProduct(item, productId));
  }

  let found = false;
  cartItems.forEach((item) => {
    if (isSameProduct(item, productId)) {
      found = true;
  ...
  if (!found) {
    cartItems.push({ product: productDoc || productId, quantity });
  }
...
const updateProductInCart = async (user, productId, quantity) => {
  ...
  const productExist = userCart.cartItems.find((item) => isSameProduct(item, productId));
  if (!productExist) {
    throw new ApiError(httpStatus.BAD_REQUEST, "Product not in cart");
  }
  userCart.cartItems = updateCartItems(userCart.cartItems, productId, quantity, product);
  await userCart.save();
  return await userCart
};
...
const deleteProductFromCart = async (user, productId) => {
  ...
  const productExist = userCart.cartItems.findIndex((item) => isSameProduct(item, productId));
  if (productExist === -1) {
    throw new ApiError(httpStatus.BAD_REQUEST, "Product not in cart");
  }
  userCart.cartItems = updateCartItems(userCart.cartItems, productId, 0);
  await userCart.save();
