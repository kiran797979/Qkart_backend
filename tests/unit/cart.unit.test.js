...
describe("Cart test", () => {
  ...
  describe("delete product in cart", () => {
    it("should delete product from cart", async () => {
      // Create a deep copy of the fixture to avoid mutation
      const cartCopy = JSON.parse(JSON.stringify(cartWithProductsUserOne));
      ...
      mockingoose(Cart).toReturn(cartCopy, "findOne");
      mockingoose(Cart).toReturn(saveMock, "save");

      const existingProduct = cartCopy.cartItems[0].product;
